...
class OptCarlini:
    ...
    def _setup_opt(self):
        ...
        if self.use_bound:
            ...
            self.optimizer = tf.train.AdamOptimizer(
                learning_rate=self.lr, beta1=0.9, beta2=0.999, epsilon=1e-08)
    ...
    def __init__(self, model, target=True, c=10, lr=0.01, init_scl=0.2,
        ...
        if loss_op == 0:
            ...
            if tf.argmax(model_output, axis=1)[0] == i_y:
                z_i = output_2max[1]
            else:
                z_i = output_2max[0]
            if self.target:
        ...
        for c_step in range(search_step):
            ...
            if self.target:
                ...
                if score > 1 - SCORE_THRES:
                    ...
                    norm_suc = norm
            if prog:
                print "c_Step: {}, score={:.3f}".format(c_step, score)
